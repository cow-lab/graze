...
import { recordComprehensionPass, type PaperRef } from "@/lib/actions/engagement";
import { markChewed } from "@/lib/chewSession";
import ReadingControls, { useReadingPrefs, TEXT_SIZE_CLASS } from "@/components/ReadingControls";
...
type LoadResult = { ok: true; explainer: ExplainerData } | { ok: false; message: string };
...
export default function ExplainerPanel({
  ...
  const allAnswered = quiz.length > 0 && answers.length === quiz.length && answers.every((a) => a !== null);
  ...
  return (
    <div>
      ...
      >
        ...
        Chew on this
        <span className="sr-only"> — read a plain-language explainer and take a comprehension check</span>
      </button>
        ...
        >
          ...
          >
            <div className="sticky top-0 z-10 flex items-center justify-between gap-3 bg-panel/95 backdrop-blur-sm border-b border-border px-5 py-3">
              ...
              >
                <Sparkles size={14} className="text-teal" aria-hidden="true" /> Chew on this
              </h2>
              <button
                type="button"
                onClick={close}
            ...
            <div className="p-5">
              <div aria-live="polite" aria-atomic="true">
                ...
                {loading && <RunningCowLoader message="Chewing on this one…" className="py-6" />}
                ...
                <>
                  ...
                  {isNonEnglish(language) && (
                    <p className="mb-3 rounded-md border border-border-strong bg-panel-2 px-3 py-2 text-xs text-fg-muted">
                      ...
                      <span className="font-medium text-fg">{languageName(language)}</span>. The
                      summary below is in English, so it&apos;s one step removed from the original
                      — worth keeping in mind if you go back to the source to check it.
                    </p>
                  )}
                  ...
                  <p
                    ...
                    style={
                      dyslexic
                        ? { fontFamily: '"OpenDyslexic", var(--font-sans), sans-serif' }
                        : undefined
                    }
                  >
                    {data.summary}
                  ...
                  {sourceUrl && (
                    <p className="-mt-2 mb-4 text-xs text-fg-muted">
                      A summary written by a model is a second-hand account. If you&apos;re going
                      to say something about this paper, say it after reading the source.
                    </p>
                  )}
                  ...
                  <div className="flex flex-col gap-4">
                    {quiz.map((q, qi) => (
                      ...
                      <fieldset key={qi} className="border-0 p-0 m-0">
                        <legend className="text-sm font-medium mb-1.5">{q.question}</legend>
